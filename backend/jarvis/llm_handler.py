...
class LLMHandler:
    ...
    def _validate_syntax(self, command: str) -> bool:
        ...
        On Windows, skip bash validation and do basic sanity checks instead
        """
        ...
        # Basic checks that work on all platforms
        if not command:
            return False
        
        # Check if it's a valid command pattern (starts with alphanumeric or common symbols)
        if not re.match(r'^[a-zA-Z0-9_\-./\\]+', command):
            return False
        
        # Reject obvious non-commands
        if command.lower().startswith(("to ", "this ", "here ", "you ", "use ", "run ", "the ")):
            return False
        
        # On Unix-like systems, try bash validation
        import subprocess
        import platform
        ...
        # On Windows or if bash validation fails, return True for basic checks
        return True
